...
# Ensure Mu_21d & Sigma_21d keys exist in the in-memory weights (avoid KeyError later)
# and persist immediately so the file and in-memory state match.
for key, default_val in (('Mu_21d', 0.10), ('Sigma_21d', 0.10)):
    if key not in weights:
        weights[key] = default_val

# persist the weights file so external scripts see the same keys
try:
    with open(WEIGHTS_FILE, 'w') as wf:
        json.dump(weights, wf, indent=2)
except Exception as e:
    print(f"⚠️ Could not persist initial weights file: {e}")

# small helper to safely read float weight values elsewhere in the code
def weight_get(k, default=0.0):
    try:
        return float(weights.get(k, default))
    except Exception:
        return float(default)

# === Reinforce Top Binary Conditions (Weekly Boost) - filtered to binary conditions only ===
try:
    perf_df = pd.read_csv("condition_performance.csv")
    # Only keep conditions that are in our binary/default set (avoid Mu/Sigma etc.)
    condition_keys = set(default_weights.keys())
    perf_bin = perf_df[perf_df['Condition'].isin(condition_keys)].copy()

    if perf_bin.empty:
        print("⚠️ No binary condition performance rows found; skipping binary boosters.")
    else:
        top_conditions = perf_bin.sort_values(by="Win_Rate_%", ascending=False).head(3)['Condition'].tolist()
        bottom_conditions = perf_bin.sort_values(by="Win_Rate_%", ascending=True).head(2)['Condition'].tolist()

        for cond in top_conditions:
            if cond in weights:
                weights[cond] = round(weights[cond] * 1.20, 4)  # Boost by 20%

        for cond in bottom_conditions:
            if cond in weights:
                weights[cond] = round(max(weights[cond] * 0.90, 0.05), 4)  # Penalize 10%, floor 0.05
    ...
    print("⚠️ condition_performance.csv not found — binary reinforcement skipped.")
except Exception as e:
    print(f"⚠️ Reinforcement failed: {e}")

# === Continuous feature performance tracker & conservative weight updater ===
# Requires scipy.stats; install if missing: pip install scipy
import scipy.stats as ss

CONT_PERF_FILE = "continuous_feature_perf.csv"

def compute_continuous_perf_and_update(ind_csv=None, price_csv=None,
                                       features=('mu_score', 'sigma_score'),
                                       horizon_days=5,
                                       lookback_days=7,
                                       lr=0.05, ema_alpha=0.25, w_min=0.01, w_max=2.0):
    """
    Compute simple performance stats (IC, quantile edge) for continuous features
    and perform a conservative, smoothed weight update for Mu/Sigma keys.

    Defaults are resolved at runtime so this function may be declared before
    IND_CSV / OHLCV_CSV are defined elsewhere in the file.
    """

    # Resolve defaults at runtime (safe if constants not yet defined)
    if ind_csv is None:
        ind_csv = globals().get('IND_CSV', 'stock_indicators_volume.csv')
    if price_csv is None:
        price_csv = globals().get('OHLCV_CSV', 'stock_prices_volume.csv')

    try:
        df_ind = pd.read_csv(ind_csv, parse_dates=['Date'])
    except Exception as e:
        print(f"⚠️ Could not read indicators for continuous perf: {e}")
        return

    try:
        df_price = pd.read_csv(price_csv, parse_dates=['Date'])
    except Exception as e:
        print(f"⚠️ Could not read prices for continuous perf: {e}")
        return
    ...
    try:
        pivot = df_price.pivot(index='Date', columns='Symbol', values='Close').sort_index()
    except Exception as e:
        print(f"⚠️ Could not build price pivot: {e}")
        return

    results_rows = []
    eval_dates = sorted(df_ind['Date'].dt.strftime('%Y-%m-%d').unique())
    if not eval_dates:
        print("⚠️ No indicator dates available for continuous perf.")
        return

    recent_dates = eval_dates[-lookback_days:]

    # map indicator feature name -> weight key in weights dict
    feature_to_weight_key = {'mu_score': 'Mu_21d', 'sigma_score': 'Sigma_21d',
                             'Mu_21d': 'Mu_21d', 'Sigma_21d': 'Sigma_21d'}

    for d in recent_dates:
        d_date = pd.to_datetime(d)
        if d_date not in pivot.index:
            # skip if price missing for that date
            continue

        # compute target date for forward horizon
        idx = pivot.index.get_loc(d_date)
        if idx + horizon_days >= len(pivot.index):
            continue
        target_date = pivot.index[idx + horizon_days]

        prices_d = pivot.loc[d_date]
        prices_t = pivot.loc[target_date]
        fwd_ret = (prices_t - prices_d) / prices_d  # series indexed by symbol

        ind_slice = df_ind[df_ind['Date'].dt.strftime('%Y-%m-%d') == d].set_index('Symbol')

        for feat in features:
            if feat not in ind_slice.columns:
                continue
            feat_series = ind_slice[feat].astype(float)
            combined = pd.DataFrame({'feat': feat_series, 'fwd': fwd_ret}).dropna()
            if combined.shape[0] < 30:
                # too small sample for stats; still record but skip update
                continue

            # Spearman & Pearson
            try:
                ic_spear = ss.spearmanr(combined['feat'], combined['fwd']).correlation
            except Exception:
                ic_spear = float('nan')
            try:
                ic_pear = ss.pearsonr(combined['feat'], combined['fwd'])[0]
            except Exception:
                ic_pear = float('nan')

            q_hi = combined['feat'].quantile(0.8)
            q_lo = combined['feat'].quantile(0.2)
            top_mean = combined[combined['feat'] >= q_hi]['fwd'].mean()
            bot_mean = combined[combined['feat'] <= q_lo]['fwd'].mean()
            edge = float((top_mean - bot_mean) if (pd.notna(top_mean) and pd.notna(bot_mean)) else 0.0)

            results_rows.append({
                'EvalDate': d,
                'Feature': feat,
                'N': int(combined.shape[0]),
                'IC_spearman': float(ic_spear) if not pd.isna(ic_spear) else None,
                'IC_pearson': float(ic_pear) if not pd.isna(ic_pear) else None,
                'TopMean': float(top_mean) if pd.notna(top_mean) else None,
                'BotMean': float(bot_mean) if pd.notna(bot_mean) else None,
                'Edge': edge
            })

            # Now update the corresponding weight conservatively
            wkey = feature_to_weight_key.get(feat)
            if not wkey or wkey not in weights:
                continue

            ic_use = ic_spear if (not pd.isna(ic_spear)) else (ic_pear if (not pd.isna(ic_pear)) else 0.0)
            # compute multiplicative update (small)
            update_factor = 1.0 + lr * max(min(ic_use, 1.0), -1.0)
            candidate = float(weights[wkey]) * update_factor
            candidate = max(w_min, min(w_max, candidate))
            new_w = ema_alpha * candidate + (1.0 - ema_alpha) * float(weights[wkey])
            new_w = max(w_min, min(w_max, new_w))

            print(f"[CONT-PERF] {feat} date={d} ic={ic_use:.4f} edge={edge:.5f} -> {wkey}:{weights[wkey]:.4f} -> {new_w:.4f}")
            weights[wkey] = float(new_w)

    # persist results and weights
    if results_rows:
        pd.DataFrame(results_rows).to_csv(CONT_PERF_FILE, index=False)
    try:
        with open(WEIGHTS_FILE, 'w') as wf:
            json.dump(weights, wf, indent=2)
    except Exception as e:
        print(f"⚠️ Could not persist updated weights after continuous perf update: {e}")
# --- Run continuous feature perf + weight update once weekly (e.g., on Monday) ---
# You can change weekday index (0=Monday .. 6=Sunday). This avoids noisy per-hour updates.
try:
    run_cont_perf_weekday = 0  # Monday
    if datetime.today().weekday() == run_cont_perf_weekday:
        compute_continuous_perf_and_update(horizon_days=5, lookback_days=7, lr=0.05, ema_alpha=0.25)
    else:
        # optional: compute but do not update weights? For now we skip entirely on other days.
        pass
except Exception as e:
    print(f"⚠️ Continuous perf run failed: {e}")


# --- Alpaca API ---
API_KEY = 'PKAXRFY0UIMH6Q91FGTH'
API_SECRET = 'c0gkYh1aspdySLc8XPs5FodUfiZDXUdDo5yqtRMn'
BASE_URL = 'https://paper-api.alpaca.markets'
api = tradeapi.REST(API_KEY, API_SECRET, BASE_URL, api_version='v2')

# --- User Config ---
STOCK_CSV = r'C:\Users\DELL\Desktop\testing\Dowjones_symbols.csv'
MAX_BUDGET = 500.0
MIN_HOLD_DAYS = 1
MAX_HOLD_DAYS = 7
# Banking settings
...
try:
    ...
    def scaled_from_z(x, mean, std):
        # robust: return neutral 0.5 for NaN, otherwise clipped z -> [0,1]
        try:
            if pd.isna(x):
                return 0.5
            z = (float(x) - float(mean)) / float(std)
            if z != z:  # catches NaN
                return 0.5
            # clip
            if z < -3.0:
    ...
    # -------------------------
    # Use learned weights (and ensure Mu/Sigma keys exist)
    # -------------------------
    # If your code earlier loaded 'weights' from WEIGHTS_FILE, prefer it.
    # Fallback to default_weights if not present.
    try:
        weight_dict = weights.copy()
    except Exception:
        weight_dict = default_weights.copy()

    # Ensure Mu_21d and Sigma_21d are present in weights (give conservative defaults)
    if 'Mu_21d' not in weight_dict:
        weight_dict['Mu_21d'] = 0.10  # default; tune later
    if 'Sigma_21d' not in weight_dict:
        weight_dict['Sigma_21d'] = 0.10

    # Save updated learned weights back to disk so future runs see Mu/Sigma keys.
    # Use the same weights file variable your bot uses (WEIGHTS_FILE)
    try:
        if 'WEIGHTS_FILE' in globals():
            with open(WEIGHTS_FILE, 'w') as wf:
                json.dump(weight_dict, wf, indent=2)
        weights = weight_dict.copy()
    except Exception as e:
        print(f"⚠️ Could not persist updated weights file: {e}")

    # Now extract continuous weights from the weight dict to use in scoring
    w_mu = float(weight_dict.get('Mu_21d', 0.10))
    w_sigma = float(weight_dict.get('Sigma_21d', 0.10))

    # Binary condition weights will be the weight_dict entries for the condition keys.
    # Build binary_weights mapping by filtering weight_dict for your condition keys.
    # (This allows Mu/Sigma to live in the same JSON alongside condition weights.)
    condition_keys = list(default_weights.keys())
    binary_weights = {k: float(weight_dict.get(k, default_weights.get(k, 0.0))) for k in condition_keys}
...
if not top_picks.empty:
    dynamic_thresh = max(0.50, top_picks['Total_Score'].quantile(0.85))  # top 15% or at least 0.50
...
# ===========================
# ---------- Trade-based learning: update deltas for binary conditions ----------
# ---------- Trade-based learning: update deltas for binary conditions ----------
try:
    ...
    # Initialize deltas for all currently-known weight keys (avoids KeyError)
    deltas = {k: 0.0 for k in weights.keys()}

    for _, row in sell_trades.iterrows():
        sym = row.get('Symbol')
        time_str = row.get('Time')

        # tolerate different column names for PnL/spread in trade log
        try:
            pnl = float(row.get('Spread', row.get('PnL', 0.0)))
        except Exception:
            pnl = 0.0

        # safely parse trade date (fall back if missing)
        trade_date = None
        try:
        ...
        if not sym or not trade_date:
            # skip rows with missing symbol or date
            continue

        # Look up indicator row for that symbol & date
        ...
        # Only update the binary condition keys (those defined in default_weights)
        # Continuous features (Mu_21d / Sigma_21d) are handled by the continuous perf updater.
        for cond in default_weights.keys():
            try:
                # accept either numeric 0/1 or boolean True/False in the indicators file
                val = match.get(cond, 0)
                if int(val) == 1:
                    # small additive credit/debit depending on PnL sign
                    deltas[cond] += (0.05 if pnl > 0 else -0.05)
            except Exception:
                # any problem reading the single condition -> skip it
                continue

    # Diagnostic: report non-zero deltas (optional)
    nonzero = {k: v for k, v in deltas.items() if abs(v) > 1e-9}
    if nonzero:
        print(f"[LEARN] Updated deltas from trade history: {nonzero}")

except FileNotFoundError:
    print(f"[LEARN] trades or indicators file missing, skipping trade-based learning.")
except Exception as e:
    print(f"[LEARN] unexpected learning error: {e}")
# -------------------------------------------------------------------------------
...
# --- helper: consistent PnL computation ---
def compute_total_pnl(price, buy_price, qty, slippage_bps=None):
    """
    Returns (total_pnl, per_share_gain).
    per_share_gain = price*(1 - slippage_bps) - buy_price
    total_pnl = per_share_gain * qty
    """
    sl = SLIPPAGE_BPS if slippage_bps is None else float(slippage_bps)
    try:
        per_share = float(price) * (1.0 - float(sl)) - float(buy_price)
        total = round(per_share * float(qty), 2)
        return total, round(per_share, 6)
    except Exception:
        return 0.0, 0.0
def ensure_trades_log_header():
    """Create trade log with header if missing."""
    header = "Action,Symbol,Time,Price,Qty,Reason,PnL\n"
    if not os.path.exists(TRADES_LOG):
        try:
            with open(TRADES_LOG, "w", encoding="utf-8", newline="") as f:
                f.write(header)
        except Exception as e:
            print(f"⚠️ Could not create trades log header: {e}")

def log_trade(action, sym, price, qty, reason, pnl):
    """
    Write a single trade line to TRADES_LOG.
    Fields: Action,Symbol,Time,Price,Qty,Reason,PnL
    """
    try:
        ensure_trades_log_header()
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        # normalize numeric types & formatting
        price_f = float(price) if price is not None else 0.0
        qty_f = float(qty) if qty is not None else 0.0
        pnl_f = float(pnl) if pnl is not None else 0.0

        line = f"{action},{sym},{now_str},{price_f:.2f},{qty_f:.6f},{reason},{pnl_f:.2f}\n"
        with open(TRADES_LOG, "a", encoding="utf-8", newline="") as f:
            f.write(line)
    except Exception as e:
        print(f"⚠️ log_trade error: {e}")
# ===========================
...
while True:
    try:
                    ...
                    try:
                        ...
                        if gain_pct < 0.1: # Breakeven or losing
                            ...
                            try:
                                ...
                                # Safely read buy price
                                try:
                                    bp = float(entry.get('Buy_Price', entry.get('BP', 0.0)))
                                except Exception:
                                    bp = 0.0

                                SLIPPAGE_BPS = globals().get('SLIPPAGE_BPS', 0.001)
                                per_share = (price * (1 - SLIPPAGE_BPS)) - bp
                                total_pnl = per_share * float(qty)

                                # Log trade using helper
                                log_trade("SELL", sym, price, qty, "ForceSell", total_pnl)
                                print(f"✅ Force-sell {sym} qty={qty} @ {price:.2f} | PnL=${total_pnl:.2f}")

                                # Remove from portfolio once
                                port_df = port_df[port_df.Symbol != sym]
                                port_df.to_csv(PORTFOLIO, index=False)
        ...
        if candidates.empty:
            ...
            dynamic_thresh = max(0.50, candidates['Total_Score'].quantile(quantile_level))  # floor at 0.60
            # Debug print so you can see the threshold each loop
            print(f"🔎 Dynamic threshold (quantile {quantile_level}): {dynamic_thresh:.3f} (floor 0.50)")
        ...
        for _, entry in port_df.iterrows():
            ...
            if remaining > 0 and price >= take_target1:
                ...
                try:
                    ...
                    SLIPPAGE_BPS = globals().get('SLIPPAGE_BPS', 0.001)
                    per_share = (price * (1 - SLIPPAGE_BPS)) - bp
                    total_pnl = per_share * float(sell_qty)

                    log_trade("SELL", sym, price, sell_qty, "TakeProfit1", total_pnl)
                    print(f"✅ Partial TP executed: {sym} sold {sell_qty} @ {price:.2f} | PnL=${total_pnl:.2f}")

                    # update remaining qty and trailing stop
                    remaining = max(0.0, remaining - sell_qty)
                    new_trailing = price * (1 - 0.03)  # set trailing 3% below current price (tunable)
            ...
            try:
                if remaining > 0 and price <= trailing_stop:
                    ...
                    log_trade("SELL", sym, price, sell_qty, "TrailingStop", total_pnl)
                    print(
                        f"💥 Trailing stop fired for {sym}, sold remaining {sell_qty} @ {price:.2f} | PnL=${total_pnl:.2f}")
            ...
            try:
                ...
                if decision == 'SELL':
                    try:
                        ...
                        log_trade("SELL", sym, price, qty, reason, total_pnl)
                        print(f"💸 Selling {sym} → Reason: {reason} | Qty: {qty} | PnL=${total_pnl:.2f}")
            ...
            for pos in positions:
                ...
                if value <= 0.01 or qty <= 0.005:
                    ...
                    try:
                        ...
                        # Log the trim
                        log_trade("TRIM", sym, price, qty, "Auto-Trim", total_pnl)

                        # Weekly profit log (total profit for this trim)
                        date_str = datetime.today().strftime('%Y-%m-%d')
                        try:
                            header2 = "Date,Symbol,PnL\n"
                            if not os.path.exists("weekly_profit_log.csv"):
                                with open("weekly_profit_log.csv", "w", encoding="utf-8", newline="") as f:
                                    f.write(header2)
                            with open("weekly_profit_log.csv", "a", encoding="utf-8", newline="") as f:
                                f.write(f"{date_str},{sym},{total_pnl:.2f}\n")
                        except Exception as e:
                            print(f"⚠️ Could not write weekly profit log: {e}")

                        # Update banked cash only if profit is positive
                        if total_pnl > 0:
                            try:
                                if not os.path.exists("banked_cash.txt"):
                                    with open("banked_cash.txt", "w", encoding="utf-8") as b:
                                        b.write("0.0")
                                with open("banked_cash.txt", "r", encoding="utf-8") as b:
                                    current_banked = float(b.read().strip() or 0.0)
                            except Exception:
                                current_banked = 0.0

                            current_banked += total_pnl
                            try:
                                with open("banked_cash.txt", "w", encoding="utf-8") as b:
                                    b.write(str(round(current_banked, 2)))
                                print(f"💰 Banked ${total_pnl:.2f} from {sym}. Total banked: ${current_banked:.2f}")
                            except Exception as e:
                                print(f"⚠️ Could not update banked cash: {e}")
                        else:
                            print(f"📉 Logged loss of ${total_pnl:.2f} for {sym}")

                        # Remove from portfolio and persist
                        port_df = port_df[port_df['Symbol'] != sym]
                        port_df.to_csv(PORTFOLIO, index=False)
