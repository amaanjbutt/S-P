...
default_weights = {
    ...
    'OBV_Positive': 0.4,
    'Gap_Up': 0.3,
    'C_5d_Positive_Momentum': 0.4,
    'C_3d_Volume_Spike': 0.4
}
...
# --- Alpaca API ---
API_KEY = 'PKMC51VBD36J6KS988AH'
API_SECRET = '5eu8dKxPceL72cW4jDc88ehM32B4AZQicVHBCOJa'
BASE_URL = 'https://paper-api.alpaca.markets'
api = tradeapi.REST(API_KEY, API_SECRET, BASE_URL, api_version='v2')

# --- User Config ---
STOCK_CSV = r'C:\Users\DELL\Desktop\Hybrid stock model\Only volume\buying on the down\Stocks_Volume_Conditions.csv'
MAX_BUDGET = 500.0
MIN_HOLD_DAYS = 2
MAX_HOLD_DAYS = 7
# Banking settings
...
def days_held(buy_date):
    """
    Return full days since buy_date. Handles:
     - None / pd.NaT -> returns a large number (treat as 'old' so sells are allowed)
     - string -> parsed via dateutil.parser
     - pandas.Timestamp -> converted
     - datetime/date -> handled
    """
    # missing/NA -> treat as long-held so we don't crash and allow banking/selling logic to proceed
    if buy_date is None or pd.isna(buy_date):
        # log a helpful warning once or when debugging
        # returning a large number avoids blocking sells due to unknown buy date
        return 999
    ...
    try:
        ...
        if isinstance(buy_date, _pd.Timestamp):
            buy_date = buy_date.to_pydatetime()
    except Exception:
        pass

    # If it's a date or datetime-like object with .date()
    ...
    # If it's already a date object (unlikely), try direct subtraction
    try:
                    ...
                    for _, row in top_buy_candidates.iterrows():
                        ...
                        try:
                            if float(qty) >= 1:
                                ...
                                reason = f"Top10 Score={row['Total_Score']} (fractional)"  # ✅ FIXED
