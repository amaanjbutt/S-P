...
# --- Alpaca API ---
API_KEY = 'PK89UFBILZFABE9EXM1M'
API_SECRET = 'iOsvwlH07R5cqqg58upsIqT88gJHCNfqR0qFzZWs'
BASE_URL = 'https://paper-api.alpaca.markets'
api = tradeapi.REST(API_KEY, API_SECRET, BASE_URL, api_version='v2')

# --- User Config ---
STOCK_CSV = r'C:\Users\DELL\Desktop\Hybrid stock model\Only volume\momentum buy\Stocks_Volume_Conditions.csv'
MOMENTUM_BUDGET = 250
MAX_BUDGET = 250.0
MIN_HOLD_DAYS = 1
MAX_HOLD_DAYS = 7
...
TRADES_LOG = 'updated_trades_log_volume1.csv'
BANKING_STATUS_FILE = "banking_status.txt"
INTRADAY_FILE = "intraday_candidates.csv"
WEEKLY_PNL_FILE = "weekly_pnl_log.csv"
MOMENTUM_FILE = "momentum_watchlist.csv"
MOMENTUM_LOG = "google_gainers_log.csv"
MOMENTUM_PORTFOLIO_CSV = "momentum_portfolio.csv"
...
# Ensure momentum portfolio exists
if not os.path.exists(MOMENTUM_PORTFOLIO_CSV):
    pd.DataFrame(columns=['Symbol', 'BuyPrice', 'Qty', 'Date']).to_csv(MOMENTUM_PORTFOLIO_CSV, index=False)
    print("🆕 Created momentum portfolio file.")
# ===========================
...
def days_held(buy_date):
    if buy_date is None or pd.isna(buy_date):
        # Return a large number so that invalid rows are always considered "old"
        return 999
    ...
    if isinstance(buy_date, str):
        try:
            ...
            return 999  # fallback if parsing fails
...
# === Fetch Google Finance Top Gainers ===
GOOGLE_GAINERS_URL = "https://www.google.com/finance/markets/gainers"
# === RESET MOMENTUM LOG ON BOT START ===
today = datetime.now().strftime("%Y-%m-%d")  # <-- define once at top

if os.path.exists(MOMENTUM_LOG):
    df = pd.read_csv(MOMENTUM_LOG)
    if not df.empty:
        df['Date'] = pd.to_datetime(df['DateTime']).dt.strftime("%Y-%m-%d")
        df = df[df['Date'] == today]  # Keep only today's data if any
        df.drop(columns=['Date'], inplace=True, errors='ignore')
        df.to_csv(MOMENTUM_LOG, index=False)
        print(f"🔄 Momentum log reset for today ({today}).")
    else:
        print("📄 Empty momentum log found — starting fresh.")
else:
    pd.DataFrame(columns=['DateTime', 'Symbol', 'Price', 'Change']).to_csv(MOMENTUM_LOG, index=False)
    print(f"🆕 Created fresh momentum log for today ({today}).")

def fetch_google_top_gainers():
    try:
        res = requests.get(GOOGLE_GAINERS_URL, timeout=10)
        if res.status_code != 200:
            print(f"⚠️ Google Finance returned {res.status_code}")
            return pd.DataFrame(columns=['Symbol', 'Price', 'Change', 'DateTime'])

        soup = BeautifulSoup(res.text, "html.parser")
        tickers = [div.get_text(strip=True) for div in soup.select("div.CUaKTb, div.COaKTb")]  # ticker class
        prices = [div.get_text(strip=True).replace(",", "") for div in soup.select("div.YMlKec")]
        changes = [div.get_text(strip=True) for div in soup.select("div.JwB6zf")]
        ...
        # Save for history
        try:
            existing = pd.read_csv(MOMENTUM_LOG)
        ...
        print(f"❌ Failed to fetch Google Finance gainers: {e}")
        return pd.DataFrame(columns=['Symbol', 'Price', 'Change', 'DateTime'])


def update_momentum_watchlist(momentum_budget=250.0, min_appearances=2):
    """
    Fetch top gainers from Google Finance, log them with timestamp,
    and filter for symbols that appeared multiple times today.

    Args:
        momentum_budget (float): Budget allocated for momentum buys.
        min_appearances (int): Minimum times a symbol must appear today to qualify.

    Returns:
        pd.DataFrame: DataFrame with qualifying momentum candidates.
    """
    # Step 1: Fetch current gainers and log them
    new_data = fetch_google_top_gainers()
    if new_data.empty:
        print("⚠️ No gainers fetched this loop — skipping momentum update.")
        return pd.DataFrame(columns=['Symbol', 'Price', 'Appearances', 'Momentum_Alloc'])

    # Step 2: Load full log
    try:
        log_df = pd.read_csv(MOMENTUM_LOG)
    except FileNotFoundError:
        print("⚠️ No momentum history found yet.")
        return pd.DataFrame(columns=['Symbol', 'Price', 'Appearances', 'Momentum_Alloc'])

    # Step 3: Filter for today's date only
    today = datetime.now().strftime("%Y-%m-%d")
    log_df['Date'] = pd.to_datetime(log_df['DateTime']).dt.strftime("%Y-%m-%d")
    ...
    counts = today_df.groupby('Symbol').size().reset_index(name='Appearances')
    counts = counts[counts['Appearances'] >= min_appearances]

    if counts.empty:
        print("⚠️ No momentum candidates qualified this loop.")
        return pd.DataFrame(columns=['Symbol', 'Price', 'Appearances', 'Momentum_Alloc'])

    # Step 5: Merge with latest prices
    latest = new_data[['Symbol', 'Price']].drop_duplicates(subset=['Symbol'])
    result = counts.merge(latest, on='Symbol', how='left')

    # Step 6: Monitor top 50 and only buy top 5
    result = result.sort_values(by="Appearances", ascending=False).head(50)
    result = result.sort_values(by="Price", ascending=False).head(7)

    # Step 7: Allocate momentum budget equally among top 5
    alloc = momentum_budget / len(result)
    result['Momentum_Alloc'] = alloc

    print(f"🔥 {len(result)} momentum candidates found — Alloc per symbol: ${alloc:.2f}")
    return result[['Symbol', 'Price', 'Appearances', 'Momentum_Alloc']]
...
while True:
    try:
        ...
        if not top_buy_candidates.empty:
                ...
                ## === SECTION 4.6b: MOMENTUM FUND BUY LOOP ===
                try:
                    momentum_candidates = update_momentum_watchlist(momentum_budget=MOMENTUM_BUDGET, min_appearances=2)

                    # === Check portfolio size before momentum buys ===
                    try:
                        port_df = pd.read_csv(MOMENTUM_PORTFOLIO_CSV)
                        current_holdings = len(port_df)
                    except FileNotFoundError:
                        current_holdings = 0

                    if current_holdings >= 10:
                        print(f"🚫 Already holding {current_holdings} stocks — max 10 allowed. Skipping momentum buys.")
                        momentum_candidates = pd.DataFrame()  # skip buy loop
                    else:
                        remaining_slots = 10 - current_holdings
                        if len(momentum_candidates) > remaining_slots:
                            momentum_candidates = momentum_candidates.head(remaining_slots)
                            print(
                                f"📊 Only {remaining_slots} slots available — trimmed to top {remaining_slots} momentum candidates.")

                    if not momentum_candidates.empty:
                        # ✅ Prefer %Change if available, fallback to Price
                        # ✅ Always sort by Price (simpler, avoids %Change errors)
                        momentum_candidates = momentum_candidates.sort_values(by="Price", ascending=False).head(5)
                        print("📊 Sorting momentum candidates by Price")

                        print(f"\n🔥 Momentum Candidates to Buy: {momentum_candidates['Symbol'].tolist()}")
                        momentum_used = 0.0

                        for _, row in momentum_candidates.iterrows():
                            sym = row["Symbol"]
                            price = float(api.get_latest_trade(sym)._raw['p'])  # Live price preferred
                            alloc = row["Momentum_Alloc"]

                            # ✅ Force allocation to stay in $25–$50 range
                            if alloc < 25:
                                alloc = 25
                            elif alloc > 50:
                                alloc = 50
                            ...
                            if cost < 25:  # safety check after rounding
                                cost = 25.0
                                qty = Decimal(str(cost / price)).quantize(Decimal('0.000001'), rounding=ROUND_DOWN)
                            ...
                            try:
                                ...
                                now_str = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                                with open(TRADES_LOG, 'a') as f:
                                    f.write(f"BUY,{sym},{now_str},{price},{qty},MomentumFund,0\n")

                                # === LOG TO MOMENTUM PORTFOLIO CSV ===
                                try:
                                    momentum_portfolio = pd.read_csv(MOMENTUM_PORTFOLIO_CSV)
                                except FileNotFoundError:
                                    momentum_portfolio = pd.DataFrame(columns=['Symbol', 'BuyPrice', 'Qty', 'Date'])

                                momentum_portfolio = pd.concat([momentum_portfolio, pd.DataFrame([{
                                    "Symbol": sym,
                                    "BuyPrice": price,
                                    "Qty": float(qty),
                                    "Date": now_str
                                ...
                                momentum_portfolio.to_csv(MOMENTUM_PORTFOLIO_CSV, index=False)

                                momentum_used += cost
                                print(
                                    f"✅ Momentum buy {sym} at ${price:.2f} (Qty {qty}) — Used ${momentum_used:.2f}/{MOMENTUM_BUDGET:.2f}"
                                )

                                if momentum_used >= MOMENTUM_BUDGET:
                                    print("💰 Momentum budget fully used, stopping buys.")
                                    break

                            except Exception as e:
                                print(f"❌ Momentum buy failed for {sym}: {e}")

                    else:
                        print("⚠️ No momentum candidates qualified this loop.")

                except Exception as e:
                    print(f"❌ Momentum loop failed: {e}")
